...
// the top. Each audit in that SBU is shown as a small square: filled
// navy = completed or currently ongoing, hollow = not started yet.
...
function SbuStatusPanel({ audits, pos }) {
  ...
  const rows = SBU_LIST.map(s => {
    ...
    const started = list.filter(a => currentAuditStage(a) !== -1);
    const notStarted = list.filter(a => currentAuditStage(a) === -1);
    return { ...s, total: list.length, startedCount: started.length, notStartedCount: notStarted.length, ordered: [...started, ...notStarted] };
  });
  return (
    <div>
            ...
            <div key={r.id} style={styles.sbuRow}>
              ...
              <div style={styles.sbuSquares}>
                {r.ordered.map(a => (
                  <div
                    key={a.id}
                    title={`${a.name} — ${currentAuditStage(a) === -1 ? "Not started" : (isComplete(a) ? "Completed" : "Ongoing")}`}
                    style={{
                      ...styles.sbuSquare,
                      background: currentAuditStage(a) !== -1 ? "#00456E" : "#fff",
                      borderColor: currentAuditStage(a) !== -1 ? "#00456E" : "#C7CEDA",
                    }}
                  />
                ))}
              ...
              <div style={styles.sbuText}>
                {r.startedCount} completed/ongoing · {r.notStartedCount} yet to begin
              </div>
...
function PrintReport({ state }) {
  ...
  const sbuRows = SBU_LIST.map(s => {
    ...
    const sns = sa.filter(a => currentAuditStage(a) === -1).length;
    const active = sa.length - sc - sns;
    return { ...s, total: sa.length, startedCount: sc + active, notStartedCount: sns };
  });
  const auditorRows = auditors.map(p => {
    const mine = audits.filter(a => a.auditor === p.name);
    const c = mine.filter(isComplete).length;
    ...
    <div style={{ ...pStyle, padding: "24px 28px", fontFamily: "Georgia, serif", fontSize: 12 }}>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: "Arial, sans-serif" }}>
        ...
        <tbody>
          {sbuRows.map(s => (
            <tr key={s.id}>
              <td style={td}>{s.name}</td><td style={td}>{s.total}</td><td style={td}>{s.startedCount}</td><td style={td}>{s.notStartedCount}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
const styles = {
  ...
  sbuText: { fontSize: 12.5, color: "#1C2733", flex: "1 1 220px", whiteSpace: "nowrap" },
