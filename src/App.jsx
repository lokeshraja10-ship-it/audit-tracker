...
// Where you should be "as on date" against the 25%-per-quarter target — a smooth
// day-by-day proportion through the FY, which lines up with the 25/50/75/100
// checkpoints at each quarter's end rather than only jumping at those boundaries.
...
function generateCsv(state) {
  const headers = ["Unit", "Audit Name", "Auditor", "Process Owner (PO)", "Current Stage",
    "Note Issue Date", "Concurrence Date", "CAR Scripts Run Date", "Pre-Draft Date", "Post Audit Discussion Date",
    "DAR Issue Date", "Final Publish Date", "Audit 360 Updation Date", "AWP Updation Date",
    "Training Done", "Training Date", "Has SAF", "No. of SAFs",
    "No. of Recommendations", "Recommendations Since Done", "Recommendations Balance",
  ...
  const rows = state.audits.map(a => {
    ...
    const recBalance = Math.max(0, recTotal - recDone);
    return [effectiveRegion(a, state.pos), a.name, a.auditor || "", a.po || "", stageLabel,
      a.noteIssueDate || "", a.concurrenceDate || "", a.carScriptsRunDate || "", a.preDraftDate || "", a.darDiscussionDate || "",
      a.darIssueDate || "", a.finalPublishDate || "", a.audit360UpdateDate || "", a.awpUpdateDate || "",
      a.trainingDone ? "Yes" : "No", a.trainingDate || "",
      a.hasSAF ? "Yes" : "No", a.hasSAF ? (a.safCount ?? "") : "",
      a.recommendationsTotal ?? "", a.recommendationsSinceDone ?? "", (a.recommendationsTotal || a.recommendationsSinceDone) ? recBalance : "",
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8, marginBottom: 10 }}>
        <div style={{ ...styles.sectionHeader, marginBottom: 0 }}>Target vs Actual Completion — {fyLabel} <span style={{ fontWeight: 400, color: "#8A93A6", fontSize: 12 }}>(25% cumulative per quarter · {totalPlanned} in scope)</span></div>
...
function UnitDetailPanel({ unit, audits, onClose, overdueThresholdDays }) {
  ...
  // 25%-per-quarter pace, minus how many actually are — the shortfall against plan,
...
const STAGE_DATE_FIELDS = ["noteIssueDate", "carScriptsRunDate", "concurrenceDate", "preDraftDate", "darDiscussionDate", "darIssueDate", "finalPublishDate"];
const STAGE_DATE_LABELS = {
  noteIssueDate: "Note Issue", carScriptsRunDate: "CAR Scripts Run", concurrenceDate: "Concurrence",
...
function RecommendationsBlock({ audit, updateAudit }) {
  ...
  const balance = Math.max(0, total - sinceDone);
  return (
    <div>
...
function AuditorPerformance({ state }) {
  ...
  const rows = useMemo(() => visibleAuditors.map(p => {
    ...
    const recommendationsBalance = Math.max(0, recommendationsTotal - recommendationsSinceDone);
    ...
    return {
      ...
      safTotal, recommendationsTotal, recommendationsSinceDone, recommendationsBalance, improvementsCount, improvementsAudits,
    };
