...
// so the exported file is a complete, faithful record of what an
// auditor can see/edit on that screen, not just a subset.
// ------------------------------------------------------------
function generateCsv(state) {
  const headers = [
    "Unit", "Audit Name", "Auditor", "Process Owner (PO)", "Current Stage",
    // --- main milestone dates (top of Update page) ---
    ...
    // --- System/Process Improvement block ---
  ...
  const rows = state.audits.map(a => {
    ...
    return [
      effectiveRegion(a, state.pos), a.name, a.auditor || "", a.po || "", stageLabel,
