...
import {
  ...
  Store, Flame, Plane, Droplet, BarChart3, Receipt, Wind, MoreHorizontal,
} from "lucide-react";
import {
...
// SBU / Entity master list — used for the SBU-wise dashboard panel and the
// SBU assignment dropdown in Setup → Audits. "Others" is the catch-all
// bucket, and any audit without an sbu assigned yet also falls into it.
const SBU_LIST = [
  { id: "Retail", name: "Retail", icon: Store },
  { id: "LPG", name: "LPG", icon: Flame },
  { id: "Aviation", name: "Aviation", icon: Plane },
  { id: "HRS", name: "HRS", icon: Users },
  { id: "Lubes", name: "Lubes", icon: Droplet },
  { id: "I&C", name: "I&C", icon: BarChart3 },
  { id: "GST", name: "GST", icon: Receipt },
  { id: "Gas", name: "Gas", icon: Wind },
...
function effectiveSbu(audit) {
  return audit.sbu || "Others";
}
// Formats an audit's location visits into one readable CSV-safe cell, e.g.
// "Terminal (12-06-2026 to 15-06-2026); Depot (20-06-2026 to -)"
function formatLocationVisitsForExport(audit) {
  const visits = audit.locationVisits || [];
  if (!visits.length) return "";
  return visits.map(v => {
    const start = v.startDate !== undefined ? v.startDate : (v.date || "");
    const end = v.endDate || "";
    const startStr = start ? fmtDateShort(start) : "-";
    const endStr = end ? fmtDateShort(end) : "-";
    return `${v.type || "Other"} (${startStr} to ${endStr})`;
  }).join("; ");
}
function generatePdfReport(state) {
  ...
  const sbuRows = SBU_LIST.map(s => {
    const sa = audits.filter(a => effectiveSbu(a) === s.id);
    const sc = sa.filter(isComplete).length;
    const sns = sa.filter(a => currentAuditStage(a) === -1).length;
    const active = sa.length - sc - sns;
    return [s.name, sa.length, sc, active, sns];
  });
  doc.setFontSize(12);
  doc.text("SBU / Entity-wise Status", 14, doc.lastAutoTable.finalY + 10);
  autoTable(doc, {
    startY: doc.lastAutoTable.finalY + 14,
    head: [["SBU / Entity", "Total", "Completed", "Ongoing", "Yet to Begin"]],
    body: sbuRows,
    headStyles: { fillColor: NAVY_RGB },
    styles: { fontSize: 9 },
  });
...
// ------------------------------------------------------------
// CSV export — mirrors EVERY field found on the Update page
// (AuditEditor), in the same top-to-bottom order it appears there,
// plus the SBU/Entity master-data field assigned in Setup.
// ------------------------------------------------------------
function generateCsv(state) {
  const headers = [
    "Unit", "Audit Name", "Auditor", "Process Owner (PO)", "SBU / Entity", "Current Stage",
    "Note Issue Date", "Pre-Draft Date", "Post Audit Discussion Date",
    "DAR Issue Date", "Concurrence Date", "Final Publish Date",
    "Audit 360 Updation Date", "AWP Updation Date",
    "CAR Scripts Run Date",
    "Number of Location Visits", "Location Visits (Type & Dates)",
    "Training Done", "Training Date",
    "HC/NHC Stock Check", "Stock Check Date",
    "Has SAF", "No. of SAFs",
  ...
  const rows = state.audits.map(a => {
    ...
    const visits = a.locationVisits || [];
    return [
      effectiveRegion(a, state.pos), a.name, a.auditor || "", a.po || "", a.sbu || "", stageLabel,
      a.noteIssueDate || "", a.preDraftDate || "", a.darDiscussionDate || "",
      a.darIssueDate || "", a.concurrenceDate || "", a.finalPublishDate || "",
      a.audit360UpdateDate || "", a.awpUpdateDate || "",
      a.carScriptsRunDate || "",
      visits.length || "", formatLocationVisitsForExport(a),
...
const ADMIN_SETUP_PIN = "bpcl-admin-2026";
/* ============================================================
   AUTH GATE
   ============================================================ */
...
function Dashboard({ state }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <SbuStatusPanel audits={audits} pos={state.pos} />
      <OverdueTable audits={audits} overdueThresholdDays={overdueThresholdDays} pos={state.pos} />
    </div>
  );
}
// ------------------------------------------------------------
// SBU / Entity-wise Status — one row per SBU, with a region filter at
// the top. Each audit in that SBU is shown as a small square using
// THREE distinct colours — completed, ongoing, and not-started are
// tracked and displayed as separate counts (they are different things,
// not one combined "completed/ongoing" bucket).
//   navy filled  = Completed (final report published)
//   blue filled  = Ongoing (started, not yet completed)
//   hollow       = Not started yet
// Audits without an SBU assigned in Setup fall into "Others".
// ------------------------------------------------------------
function SbuStatusPanel({ audits, pos }) {
  const [regionFilter, setRegionFilter] = useState("ALL");
  const scoped = regionFilter === "ALL" ? audits : audits.filter(a => effectiveRegion(a, pos) === regionFilter);
  const rows = SBU_LIST.map(s => {
    const list = scoped.filter(a => effectiveSbu(a) === s.id);
    const completedList = list.filter(isComplete);
    const notStartedList = list.filter(a => currentAuditStage(a) === -1);
    const ongoingList = list.filter(a => currentAuditStage(a) !== -1 && !isComplete(a));
    // Order squares: completed first, then ongoing, then not-started — so the
    // filled/hollow pattern reads left-to-right in a stable, predictable way.
    const ordered = [...completedList, ...ongoingList, ...notStartedList];
    return {
      ...s, total: list.length,
      completedCount: completedList.length, ongoingCount: ongoingList.length, notStartedCount: notStartedList.length,
      ordered,
    };
  });
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8, marginBottom: 10 }}>
        <div style={{ ...styles.sectionHeader, marginBottom: 0 }}>SBU / Entity-wise Status</div>
      ...
      <div style={styles.sbuPanel}>
        <div style={styles.sbuLegendRow}>
          <LegendDot color="#00456E" label="Completed" />
          <LegendDot color="#007BC9" label="Ongoing" />
          <LegendDot color="#fff" label="Yet to begin" />
        </div>
        {rows.map(r => {
          const Icon = r.icon;
          return (
            <div key={r.id} style={styles.sbuRow}>
              <div style={styles.sbuIconWrap}><Icon size={17} color="#fff" strokeWidth={2} /></div>
              <div style={styles.sbuName}>{r.name}</div>
              <div style={styles.sbuSquares}>
                {r.ordered.map(a => {
                  const complete = isComplete(a);
                  const started = currentAuditStage(a) !== -1;
                  const bg = complete ? "#00456E" : (started ? "#007BC9" : "#fff");
                  const border = complete ? "#00456E" : (started ? "#007BC9" : "#C7CEDA");
                  const label = complete ? "Completed" : (started ? "Ongoing" : "Not started");
                  return (
                    <div
                      key={a.id}
                      title={`${a.name} — ${label}`}
                      style={{ ...styles.sbuSquare, background: bg, borderColor: border }}
                    />
                  );
                })}
                {r.total === 0 && <span style={{ fontSize: 11.5, color: "#C7CEDA", fontStyle: "italic" }}>none yet</span>}
              </div>
              <div style={styles.sbuText}>
                {r.completedCount} completed · {r.ongoingCount} ongoing · {r.notStartedCount} yet to begin
              </div>
              <div style={styles.sbuTotal}>{r.total} total</div>
            </div>
          );
        })}
      </div>
...
function QuarterlyTarget({ audits, pos }) {
  ...
  const quarterEnds = [
    new Date(fyStartYear, 5, 30),
    new Date(fyStartYear, 8, 30),
    new Date(fyStartYear, 11, 31),
    new Date(fyStartYear + 1, 2, 31),
  ];
...
function LegendDot({ color, label }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11.5, color: "#5A6478" }}>
      <span style={{ width: 7, height: 7, borderRadius: 2, background: color, display: "inline-block", border: color === "#fff" ? "1.5px solid #C7CEDA" : "none" }} />
      {label}
    </div>
  );
}
...
function AuditEditor({ audit, state, updateAudit }) {
  ...
  return (
    <div style={styles.editorCard}>
      <div style={styles.editorHead}>
        <div>
          ...
          <div style={styles.editorMeta}>
            <UnitTag id={effectiveRegion(audit, state.pos)} /> · {UNITS.find(u => u.id === effectiveRegion(audit, state.pos))?.name}
            {audit.sbu && <> · <span style={{ color: "#007BC9" }}>{audit.sbu}</span></>}
          </div>
        </div>
        <StageBadge stageIdx={stageIdx} />
      </div>
      ...
      <p style={{ fontSize: 11.5, color: "#A3ABBB", margin: "-10px 0 0" }}>Auditor, PO and SBU assignment are managed by your admin under Setup.</p>
...
function PrintReport({ state }) {
  ...
  const sbuRows = SBU_LIST.map(s => {
    ...
    const sns = sa.filter(a => currentAuditStage(a) === -1).length;
    const active = sa.length - sc - sns;
    return { ...s, total: sa.length, completedCount: sc, ongoingCount: active, notStartedCount: sns };
  });
  const auditorRows = auditors.map(p => {
    const mine = audits.filter(a => a.auditor === p.name);
    const c = mine.filter(isComplete).length;
  ...
  return (
    <div style={{ ...pStyle, padding: "24px 28px", fontFamily: "Georgia, serif", fontSize: 12 }}>
      ...
      <h2 style={{ fontSize: 15, borderBottom: "2px solid #00456E", paddingBottom: 4, marginTop: 24 }}>SBU / Entity-wise Status</h2>
      <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: "Arial, sans-serif" }}>
        <thead><tr>{["SBU / Entity", "Total", "Completed", "Ongoing", "Yet to Begin"].map(h => <th key={h} style={th}>{h}</th>)}</tr></thead>
        <tbody>
          {sbuRows.map(s => (
            <tr key={s.id}>
              <td style={td}>{s.name}</td><td style={td}>{s.total}</td><td style={td}>{s.completedCount}</td>
              <td style={td}>{s.ongoingCount}</td><td style={td}>{s.notStartedCount}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
const STAGE_COLS = [
  "noteIssueDate", "preDraftDate", "darDiscussionDate", "darIssueDate",
  "concurrenceDate", "finalPublishDate", "audit360UpdateDate", "awpUpdateDate",
];
const STAGE_COL_LABELS = [
  "Note Issue", "Pre-Draft", "DAR Discussion", "DAR Issue",
  "Concurrence", "Final Publish", "Audit 360 Updation", "AWP Updation",
];
  ...
  return (
    <div style={styles.panel}>
      ...
      <p style={{ fontSize: 12, color: "#8A93A6", marginTop: -8, marginBottom: 14 }}>
        Backfill or catch up ALL milestone dates for many existing audits in one paste — matched by exact audit name within the unit you pick below.
      ...
      <div style={{ marginTop: 12 }}>
        <textarea
          ...
          placeholder={"One audit per line, tab-separated:\nAudit Name<TAB>Note Issue<TAB>Pre-Draft<TAB>DAR Discussion<TAB>DAR Issue<TAB>Concurrence<TAB>Final Publish<TAB>Audit 360 Updation<TAB>AWP Updation\n\nLeave a date blank to skip it. Dates can be DD-MM-YYYY, YYYY-MM-DD, or like \"21 Aug 2026\".\nCopy straight from a 9-column Excel selection (audit name must match exactly, including punctuation)."}
          style={styles.textarea}
          rows={8}
        />
...
function AuditsAdmin({ state, persist }) {
  ...
  const importPasted = () => {
    ...
    const newAudits = lines.map(line => {
      ...
      return {
        id: uid(), name, unit, auditor, po, sbu: "",
  ...
  const addManual = () => {
    ...
    const a = {
      id: uid(), name: manualName.trim(), unit, auditor: "", po: "", sbu: "",
      noteIssueDate: "", locationVisits: [], preDraftDate: "",
      darDiscussionDate: "", darIssueDate: "", finalPublishDate: "",
      createdAt: new Date().toISOString(),
    };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={styles.panel}>
        ...
        <p style={{ fontSize: 12, color: "#8A93A6", marginTop: -8, marginBottom: 14 }}>
          Assign each audit's Auditor, PO and SBU/Entity here. SBU/Entity drives the "SBU / Entity-wise Status" panel on the Dashboard.
        </p>
        ...
        <div style={{ paddingLeft: 26, display: "flex", flexDirection: "column", gap: 8, marginTop: 6 }}>
          ...
          {audits.map(a => (
            <div key={a.id} style={styles.assignRow} className="fade-row">
              ...
              <select value={a.sbu || ""} onChange={e => assignAudit(a.id, { sbu: e.target.value })} style={{ ...styles.select, ...styles.assignSelect }}>
                <option value="">Assign SBU / Entity…</option>
                {SBU_LIST.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
              </select>
...
const styles = {
  ...
  // --- SBU / Entity-wise Status panel (Dashboard) ---
  sbuPanel: { background: "#fff", border: "1px solid #E2E6ED", borderRadius: 12, padding: "8px 16px", display: "flex", flexDirection: "column" },
  sbuLegendRow: { display: "flex", gap: 16, flexWrap: "wrap", padding: "8px 0", borderBottom: "1px solid #EFF3F8" },
  sbuRow: { display: "flex", alignItems: "center", gap: 14, padding: "12px 0", borderBottom: "1px solid #EFF3F8", flexWrap: "wrap" },
  sbuIconWrap: { width: 34, height: 34, borderRadius: "50%", background: "#00456E", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 },
  sbuName: { fontSize: 13.5, fontWeight: 600, color: "#1C2733", width: 90, flexShrink: 0 },
  sbuSquares: { display: "flex", flexWrap: "wrap", gap: 5, flex: "1 1 180px", minWidth: 120 },
  sbuSquare: { width: 17, height: 17, borderRadius: 4, border: "1.5px solid", flexShrink: 0 },
  sbuText: { fontSize: 12.5, color: "#1C2733", flex: "1 1 260px", whiteSpace: "nowrap" },
  sbuTotal: { fontSize: 12.5, fontStyle: "italic", color: "#5A6478", marginLeft: "auto", flexShrink: 0 },
};
