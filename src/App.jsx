...
function QuarterlyTarget({ audits, pos }) {
  ...
  // Region here means "the region this audit belongs to for review purposes" — since
  // auditors/POs can now be assigned across regions, that's the assigned PO's own home
  // region, not necessarily the audit's unit tag.
  const poRegion = useMemo(() => new Map((pos || []).map(p => [p.name, p.unit])), [pos]);
  const regionAudits = regionFilter === "ALL" ? audits : audits.filter(a => poRegion.get(a.po) === regionFilter);

  const fyAudits = regionAudits.filter(a => {
    const ref = new Date(a.noteIssueDate || a.createdAt || now);
    return ref >= fyStart && ref <= fyEnd;
  });
  const totalPlanned = fyAudits.length || regionAudits.length; // fallback if FY tagging is sparse
  ...
  const pool = fyAudits.length ? fyAudits : regionAudits;
...
function BulkStatusUpdate({ state, persist }) {
  ...
  const applyUpdate = () => {
    ...
    for (const line of lines) {
      ...
      const idx = audits.findIndex(a => a.unit === unit && a.name === name);
